...
class CareReport {   
  ...
  factory CareReport.fromJson(Map<String, dynamic> json) {     
    List<CareAcknowledgement> parsedAcks = [];
    if (json['Acknowledgements'] != null) {
      parsedAcks = (json['Acknowledgements'] as List<dynamic>).map((a) => CareAcknowledgement.fromJson(a)).toList();
    }
    ...
    return CareReport(       
      ...
      category: ReportCategory.values.firstWhere(
        (e) => e.name.toLowerCase() == (json['Category'] ?? '').toString().toLowerCase(),
        orElse: () => ReportCategory.dailyLog,
      ),
      text: json['Observations'] ?? json['DailyActivities'] ?? '',       
      photoUrls: json['PhotoUrl'] != null && json['PhotoUrl'].toString().isNotEmpty ? [json['PhotoUrl']] : [],       
      timestamp: DateTime.tryParse(json['DatetimeCreated'] ?? '') ?? DateTime.now(),     
      acknowledgements: parsedAcks,
    );   
  }
}
