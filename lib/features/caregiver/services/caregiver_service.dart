...
class CaregiverService {      
  ...
  Future<Map<String, dynamic>> editMedication({
    ...
    final response = await _client.put(
      ...
      headers: _headers(token),
      body: jsonEncode({
        'medicationName': medicationName, 'dosage': dosage, 'scheduledDate': scheduledDate,
        'scheduledTime': scheduledTime, 'category': category, 'frequency': frequency, 'notes': notes,
      }),
    );
    return _parseResponse(response);
  }
  ...
  Future<Map<String, dynamic>> editCareReport({
    ...
    final response = await _client.put(
      ...
      headers: _headers(token),
      body: jsonEncode({
        'category': category, 'healthStatusNotes': healthStatusNotes,
        'dailyActivities': dailyActivities, 'observations': observations, 'photoUrl': photoUrl,
      }),
    );
    return _parseResponse(response);
  }
